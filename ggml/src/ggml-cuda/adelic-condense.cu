    ...
    float threshold) 
{
    ...
    size_t k_offset = stream_idx * k_nb3 + token_idx * k_nb2 + head_idx * k_nb1;
    
    size_t v_offset = stream_idx * v_nb3 + token_idx * v_nb2 + head_idx * v_nb1;
    size_t prev_v_offset = stream_idx * v_nb3 + (token_idx - 1) * v_nb2 + head_idx * v_nb1;

    half * k_ptr = (half *)(k_cache + k_offset);
    half * v_ptr = (half *)(v_cache + v_offset);
    half * prev_v_ptr = (half *)(v_cache + prev_v_offset);
    ...
    float v_curr = __half2float(v_ptr[dim_idx]);
    float v_prev = __half2float(prev_v_ptr[dim_idx]);
    ...
    if (s_norm_curr > 0.0f && s_norm_prev > 0.0f) {
        ...
        if (cos_sim > threshold) {
            k_ptr[dim_idx] = __float2half(-1e4f);
        }
    }
}
    ...
    float threshold) 
{
    ...
    float * k_ptr = (float *)(k_cache + k_offset);
    float * v_ptr = (float *)(v_cache + v_offset);
    float * prev_v_ptr = (float *)(v_cache + prev_v_offset);
    ...
    float v_curr = v_ptr[dim_idx];
    float v_prev = prev_v_ptr[dim_idx];
